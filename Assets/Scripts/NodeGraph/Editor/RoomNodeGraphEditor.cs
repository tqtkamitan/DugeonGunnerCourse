...
public class RoomNodeGraphEditor : EditorWindow
{
    ...
    [MenuItem("Room Node Graph Editor", menuItem = "Window/Dugeon Editor/Room Node Graph Editor")]
    ...
    /// <summary>
    /// Open the room node graph editor window if a room node graph scriptable onject asset is double clicked in the inspector
    /// </summary>
    ...
    private void DrawBackgroundGrid(float gridSize, float gridOpacity, Color gridColor)
    {
        ...
        {
            Handles.DrawLine(
                new Vector3(gridSize * i, -gridSize, 0) + gridOffset, new Vector3(gridSize * i, position.height + gridSize, 0f) + gridOffset);
        }

        for (int j = 0; j < horizontalLineCount; j++)
        {
            Handles.DrawLine(
                new Vector3(-gridSize, gridSize * j, 0) + gridOffset, new Vector3(position.width + gridSize, gridSize * j, 0f) + gridOffset);
        }
    ...
    private void ProcessEvents(Event currentEvent)
    {
        ...
        // if mouse isn't over a room node or we currenly dragging a line from the room node then process graph event
    ...
    /// <summary>
    /// Check to see the mouse is over the room node - if so then return the room node else return null
    /// </summary>
    ...
    /// <summary>
    /// Process mouse drag event - draw line
    /// </summary>
